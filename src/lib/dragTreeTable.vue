<template>
    <div  :style="{minWidth: widthSum,overflow: 'auto'}" class="drag-tree-table" ref="table" v-bind:class="{border:border !== undefined}">
    ...
    computed: {
      widthSum(){
        ...
        this.data.columns.forEach(item=>{
          width += (item.width && Number(item.width)  !== 0) ? Number(item.width) : 160
          width += 1
        })
        return width+'px'
      },
